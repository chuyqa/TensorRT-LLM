...
from weight import load_from_hf_mistral  # isort:skip
...
def load_mistral_config(model_dir: Union[str, Path]) -> MistralConfig:
    """ Helper utility to laod MistralConfig.

    A pretrained checkpoint from modeling_RW.py has a different structure
    and is not compatible with `transformers.MistralConfig` and
    `transformers.MistralModel`. We need to manually set the config values.
    """
    ...
    if config.model_type == 'mistral':
        # Case 1. mistralai/Mistral-7B-Instruct-v0.1 
...
def parse_arguments():
    ...
    parser.add_argument('--parallel_attention',
                        action='store_true',
                        help='Use Mistral parallel attention.')
    parser.add_argument('--new_decoder_architecture',
                        action='store_true',
                        help='Use the new Mistral decoder architecture. '
    ...
    if args.model_dir is not None:
        hf_config = load_mistral_config(args.model_dir)
        ...
        # MistralConfig sets num_kv_heads by num_heads if not provided, even
...
def build_rank_engine(builder: Builder,
    ...
    tensorrt_llm_mistral = tensorrt_llm.models.MistralForCausalLM(
    ...
    if args.enable_fp8 or args.fp8_kv_cache:
        ...
        quant_scales = get_scaling_factors(args.quantized_fp8_model_path,
                                           num_layers=args.n_layer,
                                           quant_mode=args.quant_mode)
        tensorrt_llm_mistral = fp8_quantize(tensorrt_llm_mistral,
                                           quant_mode=args.quant_mode,
                                           quant_scales=quant_scales)
    if args.model_dir is not None:
        logger.info(f'Loading HF Mistral ... from {args.model_dir}')
        tik = time.time()
        if not args.load_by_shard:
            hf_mistral = AutoModelForCausalLM.from_pretrained(
                args.model_dir, trust_remote_code=True)
            load_from_hf_mistral(tensorrt_llm_mistral,
                                hf_mistral,
                                mapping,
                                dtype=args.dtype)
            del hf_mistral
        else:
            load_from_hf_checkpoint(tensorrt_llm_mistral,
                                    args.model_dir,
                                    mapping,
                                    dtype=args.dtype)
        tok = time.time()
        t = time.strftime('%H:%M:%S', time.gmtime(tok - tik))
        logger.info(f'HF Mistral loaded. Total time: {t}')
    ...
    with net_guard(network):
        # Prepare
        network.set_named_parameters(tensorrt_llm_mistral.named_parameters())
        ...
        if args.enable_debug_output:
            ...
            for k, v in tensorrt_llm_mistral.named_network_outputs():
                v = v.trt_tensor
                v.name = k
