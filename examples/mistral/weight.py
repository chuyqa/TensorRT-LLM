...
def split_qkv_weight(trtllm_mistral: tensorrt_llm.models.MistralModel,
                     weight: np.ndarray,
                     tp_size: int,
                     rank: int,
                     is_bias: bool,
    ...
    n_heads = trtllm_mistral.num_heads
    hidden_size = trtllm_mistral.hidden_size
...
def load_from_hf_mistral(trtllm_mistral: tensorrt_llm.models.MistralForCausalLM,
                        hf_mistral,
                        mapping=Mapping(),
                        dtype: Union[str, torch.dtype] = torch.float32):
    logger.info('Loading weights from HF Mistral...')
    tik = time.time()

    model_params = dict(hf_mistral.named_parameters())
    if isinstance(dtype, str):
        dtype = tensorrt_llm._utils.str_dtype_to_torch(dtype)
    num_kv_heads = trtllm_mistral.num_kv_heads

    layers_range = trtllm_mistral.get_transformer_layers(
        trtllm_mistral.mapping, trtllm_mistral.num_layers)
    for i in layers_range:
        prefix = f'transformer.h.{i}'
        layer = trtllm_mistral.layers[i - layers_range[0]]
        qkv_weight, qkv_bias = get_weight_and_bias(
            model_params, f'{prefix}.self_attention.query_key_value', dtype)
        qkv_w = split_qkv_weight(trtllm_mistral,
                                 qkv_weight,
                                 mapping.tp_size,
                                 mapping.tp_rank,
                                 is_bias=False,
        ...
        if trtllm_mistral.new_decoder_architecture:
            ...
            if not trtllm_mistral.parallel_attention:
    ...
    if mapping.is_first_pp_rank():
        trtllm_mistral.embedding.weight.value = embed_w.copy()
    if mapping.is_last_pp_rank():
        trtllm_mistral.lm_head.weight.value = split_matrix(embed_w,
                                                          mapping.tp_size,
                                                          mapping.tp_rank,
                                                          dim=0)
        ...
        ln_f_w, ln_f_b = get_weight_and_bias(model_params, 'transformer.ln_f',
                                             dtype)
        trtllm_mistral.ln_f.weight.value = ln_f_w
        if ln_f_b is not None:
            trtllm_mistral.ln_f.bias.value = ln_f_b
...
def load_from_hf_checkpoint(
        trtllm_mistral: tensorrt_llm.models.MistralForCausalLM,
    ...
    for model_file in iterate_shard_files(model_dir, mapping.tp_rank):
        ...
        for name, param in state_dict.items():
            ...
            if 'self_attention.query_key_value' in name:
                ...
                if mapping.is_first_pp_rank():
                    trtllm_mistral.embedding.weight.value = param.copy()
                if mapping.is_last_pp_rank():
                    trtllm_mistral.lm_head.weight.value = split_matrix(
                        param, mapping.tp_size, mapping.tp_rank, dim=0)
            elif 'ln_f' in name:
                if mapping.is_last_pp_rank():
                    if not is_bias(name):
                        trtllm_mistral.ln_f.weight.value = param
                    else:
                        trtllm_mistral.ln_f.bias.value = param
        del state_dict
...
def get_scaling_factors(
    ...
    """ Get the scaling factors for Mistral model

    Returns a dictionary of scaling factors for the selected layers of the
    Mistral model.

    Args:
        model_path (str): Path to the quantized Mistral model
