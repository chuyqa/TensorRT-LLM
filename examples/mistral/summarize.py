...
def TRTMistral(args, config):
    ...
    engine_name = get_engine_name('mistral', dtype, tp_size, pp_size,
                                  runtime_rank)
...
def main(args):
    ...
    if test_trt_llm:
        ...
        with open(config_path, 'r') as f:
            config = json.load(f)
        tensorrt_llm_mistral = TRTMistral(args, config)
    ...
    def summarize_tensorrt_llm(datapoint):
        ...
        if tensorrt_llm_mistral.remove_input_padding:
            line_encoded = [
        ...
        with torch.no_grad():
            tensorrt_llm_mistral.setup(batch_size,
                                      max_context_length=max_length,
                                      max_new_tokens=output_len,
                                      beam_width=num_beams)

            if tensorrt_llm_mistral.remove_input_padding:
                output_ids = tensorrt_llm_mistral.decode_batch(
                    line_encoded, sampling_config)
            else:
                output_ids = tensorrt_llm_mistral.decode(
                    line_encoded,
                    input_lengths,
                    sampling_config,
                )
        ...
        if tensorrt_llm_mistral.mapping.is_first_pp_rank():
...
if __name__ == '__main__':
    ...
    parser.add_argument('--engine_dir', type=str, default='mistral_outputs')
